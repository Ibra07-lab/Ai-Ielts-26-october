...
def parse_exercises(text):
    ...
    for q in questions[1:]:
        ...
        for opt_letter in ['A', 'B', 'C', 'D']:
            # match lines like "A) Option text" or "B) Option text ✅"
            opt_match = re.search(fr'{opt_letter}\) (.*?)$', q, re.MULTILINE)
            if opt_match:
                opt_text = opt_match.group(1).strip()
                is_correct = '✅' in opt_text
                # remove the ✅ if it exists within the text
        ...
        exercises.append({
            "sentence": sentence_original.replace('"', '\\"'),
            "targetWord": replace_this.replace('"', '\\"'),
            "options": opts
        })
...
for i, ex in enumerate(exercises_data):
    opts_str = ",\n".join([f"""                    {{ id: "{o['id']}", text: "{o['text']}", isCorrect: {'true' if o['isCorrect'] else 'false'}, feedback: "{o['feedback']}" }}""" for o in ex['options']])
    ts_objects.append(f"""            {{
                id: {i+1},
...
# I need to replace the ENTIRE existing synonymSwap array.
# The previous array started with `synonymSwap: [` and ended with `],` right before `contextTetris: [],`
# Let's use regex to replace it
content = re.sub(r'synonymSwap: \[.*?\],\s*contextTetris:', ts_replacement + '\\n        contextTetris:', content, flags=re.DOTALL)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print(f"Replaced synonym swap exercises with options format in business.ts!")
